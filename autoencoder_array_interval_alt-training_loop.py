...
def BSC_noise(inputs, e_t , batch_size):
  """ parameter : Symboles à envoyer
      return : Symboles reçus, bruités """
  x = tf.cast(inputs[0],tf.float64)
  interval = inputs[1]

  epsilon = K.reshape(tf.cast(K.argmax(interval)*e_t/4+e_t/4, tf.float32),shape=(batch_size, 1))
...
def BAC_noise(inputs, e_0_t, batch_size):
  """ parameter : Symboles à envoyer
      return : Symboles bruités + intervale crossover probability"""
  x = tf.cast(inputs[0], tf.float64)
  interval = inputs[1]
  epsilon_1_max = 0.002
  e = tf.cast(e_0_t/4, tf.float64)
...
def plot_loss(loss_dict, accuracy, val_accuracy):
  for key in loss_dict:
    plt.semilogy(loss_dict[key],label=f"Crossentropy p={key}")
...
def bit_error_rate_NN(N, k, C, Nb_sequences, e0, e1, e_t ):
  print('*******************NN-Decoder********************************************')
...
def meta_model_generator(k,channel,model_enc,model_dec,round,epsilon_t):
  ...
  if round:
    ...
    x = encoded_bits
  if channel == 'BSC':
    noisy_bits = Lambda(BSC_noise, arguments={'e_t':epsilon_t,'batch_size':batch_size}, name='noise_layer')([x,inputs_interval])
  elif channel == 'BAC':
    noisy_bits = Lambda(BAC_noise, arguments={'e_0_t':epsilon_t,'batch_size':batch_size}, name='noise_layer')([x,inputs_interval])
...
# for parameter in [0.0, 0.05]:
  ...
  if len(sys.argv) > 5:
    if sys.argv[5] == 'BER':
      ...
      C = np.round(model_encoder.predict([np.array(u_k), inter_list])).astype('int')
      BER[f"auto-array-inter_alt-{parameter}"], BLER[f"auto-array-inter_alt-{parameter}"] = bit_error_rate_NN(N, k, C, nb_pkts, e0, e1, pretrain_epsilon)
  loss_dict[parameter] = loss_values
plot_loss(loss_dict, accuracy, val_accuracy)

if len(sys.argv) > 5:
  utils.plot_BSC_BAC(f'BER N={N} k={k} - NN Array_Interval_Alternate', BER, k / N)
  utils.plot_BSC_BAC(f'BLER N={N} k={k} - NN Array_Interval_Alternate', BLER, k / N)
plt.show()
