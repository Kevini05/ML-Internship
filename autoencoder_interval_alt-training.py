...
def round_function(x):
  ...
  def grad(x):
    # return tf.gradients(tf.math.sigmoid(x),x,dy)[0]
    return tf.cond(x >= 0.0 and x<=1.0, lambda: tf.cast(1, tf.float32), lambda:tf.cast(0, tf.float32))
    # * K.switch(x<= 1 and x >=0, tf.cast(1, tf.float32), tf.cast(0, tf.float32))
  return output, grad


def gradient_stopper(x):
  output = tf.stop_gradient(tf.math.round(x)-2*x)+2*x
  # output = tf.grad_pass_through(tf.math.round(x))
  return output
...
def meta_model_generator(k,channel,model_enc,model_dec,round,train_epsilon):
  ...
  if round:
    ...
    x = encoded_bits
  if channel == 'BSC':
    noisy_bits = Lambda(BSC_noise, arguments={'train_epsilon':train_epsilon,'batch_size':batch_size}, name='noise_layer')([x,inputs_interval])
  elif channel == 'BAC':
    noisy_bits = Lambda(BAC_noise, arguments={'epsilon_0_max':train_epsilon,'batch_size':batch_size}, name='noise_layer')([x,inputs_interval])
...
epoch_encoder = 200
epoch_decoder = 500
pretrain_epsilon = 0.002
encoder_epsilon = 0.07
decoder_epsilon = 0.07
pretraining = False
...
if pretraining:
  ...
  ### Fit the model
  history = meta_model.fit([In, Interval], In, epochs=epoch_pretrain, verbose=2, shuffle=True, batch_size=batch_size, validation_data=([In, Interval], In))
  model_decoder.save(f"./autoencoder/model_decoder_non-round_interval_{channel}_rep-{rep}_epsilon-{pretrain_epsilon}_layerSize_{S}_epoch-{epoch_pretrain}_k_{k}_N-{N}.h5")
  model_encoder.save(f"./autoencoder/model_encoder_non-round_interval_{channel}_rep-{rep}_epsilon-{pretrain_epsilon}_layerSize_{S}_epoch-{epoch_pretrain}_k_{k}_N-{N}.h5")
...
for a in range(epoch):
  ...
  meta_model.compile(loss=tf.losses.softmax_cross_entropy, optimizer=keras.optimizers.Adam(1e-5), metrics=['accuracy'])

  ### Fit the model
  history = meta_model.fit([In,Interval], In, epochs=epoch_int,verbose=2, shuffle=True, batch_size=batch_size,validation_data=([In,Interval], In))
  loss_values += history.history['loss']
  accuracy += history.history['accuracy']
  val_accuracy += history.history['val_accuracy']
...
def BER_NN(nb_pkts=100):
  ...
  e0 = np.concatenate((np.linspace(0.001, 0.5, 15, endpoint=False), np.linspace(0.5, 1, 2)), axis=0)
...
# \Python3\python.exe autoencoder_interval.py BAC 8 4 500 BER 10000